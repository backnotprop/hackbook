	...
	xmlhttp.onreadystatechange = function()
	{
		if (xmlhttp.readyState == 4
			&& xmlhttp.status == 200)
			{
				...
				if (post_list.length == 0)
				{
					var tryit = "<h3>Nope, still no posts</h3>";
						document.getElementById('thefeed').innerHTML = tryit;
				}
				else
				{
	...
	xmlhttp.onreadystatechange = function()
	{
		if (xmlhttp.readyState == 4
			&& xmlhttp.status == 200)
			{
				...
				var phone = xml_doc.getElementsByTagName('phonenum')[0].textContent;
				
				userHtml += "<tr><td>Email</td><td>" + email + "</td></tr>";
				userHtml += "<tr><td>Phone</td><td>" + phone + "</td></tr>";
